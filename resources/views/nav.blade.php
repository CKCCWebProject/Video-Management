<nav class="navbar navbar-inverse navbar-fixed-top" style="background-color: {{$position=='home'?'#FF7C7C':($position=='connection'?'#7B94FD':($position=='gift'?'#FFA142':($position=='about'?'#8BAEE1':"#f1c130")))}}; border: 0px;">
    <div class="container-fluid">
        ...
        @elseif($position == 'connection')
                <div class="search-container">
                    <form>
                        <input id="search-people" class="search-box form-control" type="text" name="search" style="color: white;" placeholder="Search.." oninput="getTempPeople()">
                    </form>
                </div>
            @endif
    </div>
